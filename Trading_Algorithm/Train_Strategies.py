...
def mean_reversion_strategy(data, window=20, threshold=0.02, mode='deviation'):
    ...
    data['moving_avg'] = data['Close'].rolling(window).mean() #calculates moving average over our window
    data['signal'] = 1
    ...
    data['position'] = data['signal'].shift().fillna(0) #here his ensures that signals generated today result in a trade tomorrow
...
"""the function above picks the strategy based of lowest mse 
   however picking the best strategy based off highest cumulative 
   returns led to better results hece it is implemented below

"""

# Function to evaluate strategies and return cumulative returns
def evaluate_strategy(data, strategy_func, **kwargs):
   """
   Parameters:
...
# Training function to find the best strategy based on highest cumulative returns
def train_best_strategies(start_date, end_date):
    """
    Parameters:
...
if __name__ == "__main__":
    ...
    end_date = end_date = datetime(2024, 9, 1)
